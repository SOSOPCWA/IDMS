...
<head runat="server">
    <script type="text/javascript" language="javascript">
        function Fobj(str) { return document.getElementById("form1").elements[str]; }

        //定位坐標X 定位坐標Y 圖寬格數   圖高格數  每格長(pix) 每格寬(pix)
        X = -1    ; Y = -1;  Xno = 71; Yno = 33; Xw = 13;    Yh = 14;
        ...
        function PageLoad() {
            ...
            Fobj("BtnOK").style.posLeft = 64 * Xw;   //調整按鈕位置
            Fobj("BtnOK").style.posTop = 2 * Yh;   //調整按鈕位置            
            Fobj("BtnNo").style.posLeft = 64 * Xw;   //調整按鈕位置
            Fobj("BtnNo").style.posTop = 5 * Yh;   //調整按鈕位置
            Fobj("BtnSearch").style.posLeft = 64 * Xw;   //調整按鈕位置
            Fobj("BtnSearch").style.posTop = 8 * Yh;   //調整按鈕位置
        }
        ...
        function MouseMove()	//坐標指針	
        {
            ...
            Rx.style.left = Fobj("bigplot").style.posLeft + Fobj("bigplot").width - 24;
            Rx.style.top = window.event.y - 12;
            Ty.style.left = window.event.x - 4;
            Ty.style.top = Fobj("bigplot").style.posTop - 6;
            By.style.left = window.event.x - 4;
            By.style.top = Fobj("bigplot").style.posTop + Fobj("bigplot").height - 18;
        }
        ...
        function MapClick()	//標示選取位置
        {
            switch (IO) {
                case "I":
                    ...
                    Fobj("pos1").style.posLeft = X * Xw + 2;
                    Fobj("pos1").style.posTop = Y * Yh + 2;
                ...
                case "O":
                    ...
                    X = Math.floor((window.event.x) / Xw, 10); //int無條件捨去
                    Y = Math.floor((window.event.y) / Yh, 10); //int無條件捨去
                    Fobj("pos2").style.posLeft = X * Xw + 2;
                    Fobj("pos2").style.posTop = Y * Yh + 2;
        ...
        {
            for (j = 0; j < opener.document.all.length; j++) if (opener.document.all(j).id.indexOf("TextPointerNo")>=0) opener.document.all(j).value=PointerNo; 
        }
    </script>
</head>
...
&nbsp;<div runat="server" id='Lx' style='z-index: 1; color: red; font-weight: bold; position: absolute'>
    ...
    <input runat="server" type="button" id="BtnOK" style="z-index: 2; font-weight: bold;
        ...
        onclick="if(Y>0) {ReturnPointerNo(-(X-2)*31-Y);window.close();} else alert('您尚未定位 ! ');" />
    <input runat="server" type="button" id="BtnSearch" style="z-index: 2; font-weight: bold;
        ...
        value="查詢" onclick="window.open('../Device/Device.aspx?Search=Map&X='+(X-3)+'&Y='+Y,'_blank');" />
