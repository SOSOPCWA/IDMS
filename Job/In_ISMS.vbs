...
if Pcount>10 and Err.Number=0 then  '資產清冊連不上則不執行
  ...
  'ISMS刪除資產編號-----------------------------------------------------------------------------------------------------------------------
  rs.open "select * from [Config] where [Kind]='資產清冊' and [Config] not like '(已註銷)%' and [Item] like 'MIC-%' and [Item] not in (select [Number] from [View_資產清冊_ISMRASVR3] where [Cancel]=0)",conn,3,3
  while not rs.eof    
